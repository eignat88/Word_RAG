...
class AIProvider(EmbeddingProvider, LLMProvider, Protocol):
    pass
    ...
    def answer(self, prompt: str) -> str:
        try:
            response = httpx.post(
                ...
                json={"model": self.llm_model, "messages": [{"role": "user", "content": prompt}], "stream": False},
                headers=self._headers(),
...
def _build_openai_compatible_provider(settings: Settings) -> OpenAICompatibleProvider:
...
def _build_provider(provider_name: str, settings: Settings) -> AIProvider:
    provider = provider_name.lower()
    if provider == "ollama":
        return _build_ollama_provider(settings)
    if provider == "openai_compatible":
        return _build_openai_compatible_provider(settings)
    raise OllamaError(f"Unknown AI provider: {provider_name}")
...
def build_embedding_provider(settings: Settings) -> EmbeddingProvider:
    return _build_provider(settings.embedding_provider or settings.ai_provider, settings)


def build_llm_provider(settings: Settings) -> LLMProvider:
    return _build_provider(settings.llm_provider or settings.ai_provider, settings)
