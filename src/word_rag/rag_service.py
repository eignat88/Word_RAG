...
from .ai_provider import EmbeddingProvider, LLMProvider, build_embedding_provider, build_llm_provider
...
class RagService:
    def __init__(self, settings: Settings) -> None:
        ...
        self.embedding_ai: EmbeddingProvider = build_embedding_provider(settings)
        self.llm_ai: LLMProvider = build_llm_provider(settings)
    ...
    def ingest_directory(
        ...
        for idx, path in enumerate(files, start=1):
            ...
            embeddings = [self.embedding_ai.embed(c.chunk_text) for c in filtered_chunks]
    ...
    def search(self, question: str, fd_number: str | None = None, section: str | None = None, top_k: int | None = None) -> list[SearchResult]:
        query_emb = self.embedding_ai.embed(question)
    ...
    def answer(self, question: str, fd_number: str | None = None, section: str | None = None, top_k: int | None = None) -> dict:
        ...
        response = self.llm_ai.answer(prompt)
        return {
            "answer": response,
            "sources": [
                {
